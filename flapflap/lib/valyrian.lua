...
super = setmetatable({}, {
    ...
    __index    = function(_, value)
        return function(_, p, ...)
            local r = nil
            local instance = debug.getinfo(2, "f").func
            local mt = getmetatable(instance)

            if type(instance) ~= "class" then
                error "super may only be called from within a class method!"
            end
            ...
            for k, v in pairs(mt.__parents) do
                if v == p then
                    ...
                    if v_type == "function" then
                        local env = getfenv(_things[v][value])
                        setfenv(_things[v][value], instance)
                        r = { _things[v][value](...) }
                        setfenv(_things[v][value], env)
                        return (unpack or table.unpack)(r)
                    end
